              ...
              <div class="card">
                <div class="card-body">
                  ...
                  <div class="form-group">
                    ...
                    <datetime v-if="birthdate && !isNew" :disabled="isDisabled" v-model="birthdate"></datetime>
                    <datetime v-else v-model="birthdate"></datetime>
                  </div>
                  <div class="form-group" :class="{ 'invalid': isInvalid }">
                    <h6 class="form-label">Skills<span class="required"> *</span></h6>
                    <multiselect v-if="isNew || !isDisabled" v-model="skills" tag-placeholder="Add this as new tag" placeholder="Search or add a skill" label="name" track-by="code" :options="options" :multiple="true" :taggable="true" @tag="addTag" @close="onChangeSkills()"></multiselect>
                  ...
                  <div class="form-group" :class="{ 'invalid': isInvalidC }">
                    <h6 class="form-label">Interested In<span class="required"> *</span></h6>
                    <ul v-if="!isNew && isDisabled" class="items">
                      <li class="badge badge-secondary pull-right" v-for="(c, i) in causesList" :key="i" v-text="c"></li>
                    </ul>
                    <multiselect v-if="isNew || !isDisabled" v-model="causes" tag-placeholder="Add this as new tag" placeholder="Search or add a cause" label="name" track-by="code" :options="options2" :multiple="true" :taggable="true" @tag="addTag2" @close="onChangeCauses()"></multiselect>
                  </div>
                  ...
                  <div class="form-group" :class="{'form-group--error': $v.about.$error}">
                    <h6 class="form-label">About me<span class="required"> *</span></h6>
                    <textarea v-if="about && !isNew" :disabled="isDisabled" v-model.trim="$v.about.$model" class="form-control" rows="2"></textarea>
                    <textarea v-else class="form-control" rows="2" v-model.trim="$v.about.$model" placeholder="Introduce yourself"></textarea>
                  </div>
                  <template v-if="$v.about.$error">
                    <div class="error" v-if="!$v.about.required">
                      Field is required
                    </div>
                    <div class="error" v-if="!$v.about.minLength">
                      About must have at least
                      {{
                                $v.about.$params.minLength.min
                              }}
                      letters.
                    </div>
                  </template>
                  <div class="form-group" :class="{'form-group--error': $v.description.$error}">
                    <h6 class="form-label">Volunteering Experience<span class="required"> *</span></h6>
                    <textarea v-if="description && !isNew" v-model.trim="$v.description.$model" :disabled="isDisabled" class="form-control" rows="4"></textarea>
                    <textarea v-else class="form-control" v-model.trim="$v.description.$model" rows="4" placeholder="A short description about your volunteering experience"></textarea>
                    <template v-if="$v.description.$error">
                      <div class="error m-t-10" v-if="!$v.description.required">
                        Field is required
                      </div>
                      <div class="error m-t-10" v-if="!$v.description.minLength">
            ...
            <div class="col-xl-8">
              <div class="card">
                <div class="card-header">
                  <h4 class="card-title mb-0"><i class="fa fa-file mr-2"></i>Work History</h4>
                ...
                <div class="card-header">
                  <h4 class="card-title mb-0"><i class="fa fa-address-book-o mr-2"></i>Contact details</h4>
                  <div class="card-options"><a class="card-options-collapse" href="#" data-toggle="card-collapse"><i class="fe fe-chevron-up"></i></a><a class="card-options-remove" href="#" data-toggle="card-remove"><i class="fe fe-x"></i></a></div>
                </div>
                <div class="card-body">
                  <div class="row">
...
export default {
  ...
  methods: {
    ...mapActions(["updateVolunteer", "createVolunteer", "getProfile"]),
    getImgUrl(path) {
    ...
    serializeUser(){
      return {
        ...
        causes:this.causes.map((cause) => cause.name),
    ...
    getFormData(){
      ...
      let skills = this.skills.map((skill) => skill.name)
      for (let i = 0; i < causes.length; i++) {
        formData.append('causes[]', causes[i]);
      }
      ...
      formData.append('birthdate',new Date(this.birthdate));
      formData.append('linkedin',this.linkedin);
      return formData;
    },
    registerUser() {
      let req = this.getFormData();
      this.createVolunteer(req).then((res) => {
        if (res.data.success) {
          this.$swal({
          ...
          this.$cookie.set('volunteer',JSON.stringify(this.serializeUser()),1)
    ...
    updateUser() {
      ...
      this.updateVolunteer(req).then((res) => {
        if (res.data.success) {
          ...
          this.isNew = false;
          console.log('image',res.data.user.image)
          this.$cookie.set('volunteer-image',res.data.user.image,1)
          this.$cookie.set('volunteer',JSON.stringify(this.serializeUser()),1)
  ...
  created() {
    this.getProfile(); 
    if(this.$cookie.get('volunteer')!=null){
      this.loggedInUser = JSON.parse(this.$cookie.get('volunteer'));
      console.log(JSON.parse(this.$cookie.get('volunteer')))
      this.image = this.$cookie.get('volunteer-image');
      this.about = this.loggedInUser.about;
      this.description = this.loggedInUser.description;
      this.causesList = this.loggedInUser.causes;
      this.skillsList = this.loggedInUser.skills
      this.tags = this.loggedInUser.tags;
      this.city = this.loggedInUser.city;
      this.postcode = this.loggedInUser.postcode;
      this.birthdate = new Date(this.loggedInUser.birthdate).toISOString();
      this.mobile = this.loggedInUser.mobile;
      this.facebook = this.loggedInUser.facebook;
      this.linkedin = this.loggedInUser.linkedin;
      this.portfolio = this.loggedInUser.portfolio;
      this.isNew = false;
